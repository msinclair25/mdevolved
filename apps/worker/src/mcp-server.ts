...
function createServer(env: Env, context: ExecutionContext): McpServer {
  ...
  server.registerTool(
    "resume_project",
    {
      annotations: readOnlyAnnotations,
      description:
        "Resume an initialized Project in a fresh task. Pass projectId at the top level and the complete local .owdignore JSON as contextPolicy. For compatibility, the same exact ID may instead be present as contextPolicy.projectId. OWD automatically refreshes expiring internal context and returns it only when the policy exactly matches the owner-approved Knowledge Space pinned by this grant.",
      inputSchema: z
        .object({
