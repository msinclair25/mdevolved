...
MDevolved Sync for Obsidian vendors the YAOS Obsidian sync-client subset needed to preserve its
...
MDevolved-specific code lives in `src/**`. The semantic modifications inside the
vendored tree are deliberately narrow: the public `applyOwdConnection()` bridge
sends an already-validated MDevolved exchange result through YAOS's existing setup
controller after MDevolved's consent prompt (without a second post-exchange vault-ID
prompt); the legacy credential-bearing YAOS link/device-sharing paths and
upstream updater calls are disabled; the settings deploy link points to MDevolved;
