...
export function isObjectRecord(
  ...
  return (
    typeof value === "object" && value !== null && !(value instanceof Y.Map)
  );
}
...
export function getMetaDeletedAt(value: unknown): number | null {
  if (value instanceof Y.Map) {
    ...
    return typeof deletedAt === "number" && Number.isFinite(deletedAt)
      ? deletedAt
      : null;
  }
  if (isObjectRecord(value)) {
...
export function isFileMetaDeletedValue(value: unknown): boolean {
  ...
  if (isObjectRecord(value)) {
    ...
    if (typeof rec.deletedAt === "number" && Number.isFinite(rec.deletedAt))
      return true;
 ...
 */
export function createNestedMetaFromDecoded(
  decoded: DecodedFileMeta,
): Y.Map<unknown> {
  const entry = new Y.Map<unknown>();
...
export function computeMetaSemanticChanges(
  ...
  for (const [fileId, curr] of current) {
    ...
    const prevDeleted =
      prev.deleted === true || typeof prev.deletedAt === "number";
    const currDeleted =
      curr.deleted === true || typeof curr.deletedAt === "number";
    ...
    if (!prevDeleted && currDeleted) {
      changes.push({
        ...
        fileId,
        path: curr.path,
        deletedAt: curr.deletedAt ?? Date.now(),
      });
      continue;
    }
...
export function computeMetaShapeStats(
  ...
  metaMap.forEach((value: unknown) => {
    ...
    const isDel =
      decoded.deleted === true || typeof decoded.deletedAt === "number";
    if (isDel) {
