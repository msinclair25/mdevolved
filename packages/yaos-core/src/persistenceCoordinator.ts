...
export class PersistenceCoordinator {
  ...
  constructor(
    ...
    this.checkpointFallbackAfterFailures =
      options?.checkpointFallbackAfterFailures ??
      CHECKPOINT_FALLBACK_AFTER_FAILURES;
  ...
  enqueueSave(): Promise<SaveResult> {
    ...
    const run = this.saveChain.then(async (): Promise<SaveResult> => {
      try {
        ...
        this.health.queuedSaveCount = Math.max(
          0,
          this.health.queuedSaveCount - 1,
        );
  ...
  private async executeCheckpointFallback(
    ...
    try {
      ...
      await this.store.rewriteCheckpoint(
        checkpointUpdate,
  ...
  private async executeImmediateFallback(
    delta: Uint8Array,
  ): Promise<SaveResult> {
    try {
      const checkpointUpdate = Y.encodeStateAsUpdate(this.document);
