...
export type SvEchoParseResult =
  ...
  | {
      kind: "invalid_sv_echo";
      reason: SvEchoParseFailureReason;
      bytes: number;
    };
...
export function recordSvEchoParseResult(
  counters: SvEchoCounters,
  result: SvEchoParseResult,
): void {
...
export function parseSvEchoMessageDetailed(msg: string): SvEchoParseResult {
  ...
  try {
    ...
    return { kind: "not_sv_echo", bytes };
  }
  if (typeof parsed !== "object" || parsed === null)
    return { kind: "not_sv_echo", bytes };
  const p = parsed as Record<string, unknown>;
  if (p.type !== SV_ECHO_TYPE) return { kind: "not_sv_echo", bytes };
  if (p.schema !== SV_ECHO_SCHEMA)
    return { kind: "invalid_sv_echo", reason: "wrong_schema", bytes };
  if (typeof p.sv !== "string")
    return { kind: "invalid_sv_echo", reason: "missing_sv", bytes };
  if (p.sv.length > MAX_SV_ECHO_BASE64_BYTES)
    return { kind: "invalid_sv_echo", reason: "oversize", bytes };
  const decoded = decodeBytesBase64(p.sv);
  if (!decoded)
    return { kind: "invalid_sv_echo", reason: "invalid_base64", bytes };
  try {
