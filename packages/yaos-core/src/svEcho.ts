...
export type SvEchoSendResult =
  ...
  | {
      ...
      kind: SvEchoKind;
      bytes: number;
      failure: "not_open" | "oversize" | "send_failed";
    };
...
function framedByteLength(payload: string): number {
  return new TextEncoder().encode(`${Y_PARTYSERVER_CUSTOM_PREFIX}${payload}`)
    .byteLength;
}
...
export function trySendSvEchoStateVector(
  ...
  if (
    ...
    connection.readyState !== WS_READY_STATE_OPEN
  ) {
