...
function isChunkedManifest(value: unknown): value is CheckpointManifest {
  ...
  if (!Number.isInteger(m.chunkSizeBytes) || m.chunkSizeBytes <= 0)
    return false;
  ...
  if (typeof m.sha256 !== "string" || !/^[0-9a-f]{64}$/.test(m.sha256))
    return false;
  if (!Number.isInteger(m.stateVectorByteLength) || m.stateVectorByteLength < 0)
    return false;
  if (
    typeof m.stateVectorSha256 !== "string" ||
    !/^[0-9a-f]{64}$/.test(m.stateVectorSha256)
  )
    return false;
...
async function putChunkedPayloadBatched(
  ...
  const chunkCount =
    bytes.byteLength === 0 ? 0 : Math.ceil(bytes.byteLength / chunkSizeBytes);
  ...
  for (
    ...
    chunkStart += maxKeysPerOperation
  ) {
...
async function readChunkedPayload(
  ...
  const chunks = await getManyBatched<unknown>(
    storage,
    keys,
    maxKeysPerOperation,
  );
  ...
  for (let i = 0; i < descriptor.chunkCount; i++) {
    ...
    if (
      i < descriptor.chunkCount - 1 &&
      bytes.byteLength !== descriptor.chunkSizeBytes
    ) {
      throw new Error(
    ...
    if (offset + bytes.byteLength > descriptor.byteLength) {
      throw new Error(
        `${contextLabel} load failed: chunk bytes exceed manifest size`,
      );
    }
...
export class ChunkedDocStore {
  ...
  async rewriteCheckpoint(
    update: Uint8Array,
    stateVector: Uint8Array,
  ): Promise<void> {
    const updateBytes = normalizeBytes(update, "rewriteCheckpoint(update)");
    const stateVectorBytes = normalizeBytes(
      stateVector,
      "rewriteCheckpoint(stateVector)",
    );
    ...
    await this.storage.transaction(async (txn) => {
      ...
      if (existingPointer) {
        ...
        if (!isChunkedManifest(oldManifestRaw)) {
          throw new Error(
            `checkpoint manifest missing or invalid for version ${existingPointer.version}`,
          );
        }
      ...
      if (journalMeta.entryCount > 0) {
        ...
        for (const seq of seqs) {
          ...
          if (!isJournalEntryManifest(manifestRaw) || manifestRaw.seq !== seq) {
            throw new Error(
              `journal compact failed: invalid manifest for seq ${seq}`,
            );
          }
          cleanupKeys.add(key);
  ...
  private async loadCheckpoint(): Promise<{
    ...
    const rawManifest = await this.storage.get<unknown>(
      checkpointManifestKey(rawPointer.version),
    );
    if (!isChunkedManifest(rawManifest)) {
      throw new Error(
        `checkpoint manifest missing or invalid for version ${rawPointer.version}`,
      );
    }
    ...
    const rawStateVector = await this.storage.get<unknown>(
      checkpointStateVectorKey(rawManifest.version),
    );
    if (rawStateVector === undefined) {
      throw new Error(
        `checkpoint state vector missing for version ${rawManifest.version}`,
      );
    }
    const stateVector = normalizeBytes(
      rawStateVector,
      checkpointStateVectorKey(rawManifest.version),
    );
  ...
  private async loadJournal(): Promise<{
    ...
    if (meta.entryCount === 0) {
      if (meta.totalBytes !== 0) {
        throw new Error(
          "journal metadata is inconsistent (entryCount=0 but totalBytes>0)",
        );
      }
      return { meta, entries: [] };
    }
  ...
  private async readJournalMeta(
    source: StorageLike | TransactionLike,
  ): Promise<JournalMeta> {
