---
name: owd-eve
description: Connect, initialize, resume, or coordinate MDevolved Projects from an Eve agent using Eve's standard remote MCP connection, user-scoped Vercel Connect OAuth, qualified tool names, durable Project continuity, and independently attributable agent identities. Use when an Eve repository has agent/connections, when adding MDevolved to Eve, or when routing MDevolved work through Eve sessions, subagents, sandboxes, or schedules.
---
...
5. Use `connect()` from `@vercel/connect/eve` with
   `principalType: "user"`, `autoProvision: true`, the MDevolved MCP URL as the token
...
client-side Project authority, or a second MDevolved endpoint. Do not pin a local
...
exact MDevolved vault and folder boundary. Credentials stay in Vercel Connect rather
...
borrow a user's MDevolved grant. Dispatch scheduled MDevolved work through a
user-authenticated route or require an explicit user action. Never weaken the
MDevolved connection to app-scoped authorization merely to make a schedule run.

## Preserve identity

MDevolved attributes grants and collaboration records to the authenticated OAuth
client.

- Eve sessions and channels that reuse one connector UID are one MDevolved
  participant.
- Eve's built-in `agent` child inherits the root connection and is the same MDevolved
  participant.
- A declared local subagent discovers its own connections, but it is still the
  same MDevolved participant if it reuses the same connector UID.
...
MDevolved remains single-owner even when Eve exposes multi-user routes.
...
2. `owd__open_project`;
3. `owd__wait_for_project_connection` only when MDevolved returns a pending approval;
   and
...
MDevolved can recover the same durable request.

Persist the continuity receipt in `.owdignore` and replace only MDevolved's marked
`AGENTS.md` block, preserving all other instructions.

## Separate runtime state from Project truth

Eve's durable session preserves one runtime conversation. MDevolved remains the
portable record for the Project, Work Packet, Attempts, Artifacts, Handoffs,
Reviews, owner Decisions, citations, and provenance. Resume MDevolved at the start of
...
inspect MDevolved `localVaultAccess` before a direct write:
...
writes. MDevolved Project submission tools are append-only collaboration paths; they
