...
export const EVE_COMPATIBILITY_PROFILE = {
  ...
  projectLifecycle: {
    ...
    receipt: ".owdignore",
    rule: "Use the exact OWD lifecycle. Never ask the owner to copy a prompt, start a second OAuth flow, renew a routine packet, or create a duplicate Project.",
  },
  runtime: {
    credentials:
...
- Persist the OWD receipt in \`.owdignore\` when the workspace is durable. On a later task, resume its exact Project; do not treat Eve conversation durability as shared Project authority.
...
4. When ready, persist the continuity receipt if this workspace is durable and call \`owd__resume_project\` on later tasks.
