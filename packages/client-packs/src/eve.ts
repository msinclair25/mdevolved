...
4. When ready, persist the continuity receipt if this workspace is durable and call \`owd__resume_project\` as the first OWD action on later tasks. Until it returns, the writer role is unconfirmed; never infer it from session, channel, or sandbox identity. Treat "OWD resume project" as this receipt-based resume, with no reconnect or new approval.
