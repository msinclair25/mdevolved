...
export const OBSIDIAN_MIND_COMPATIBILITY_PROFILE = {
  ...
  toolPolicy: {
    ...
    rule: "Obsidian Mind write tools are direct vault writes. Only OWD localVaultAccess primary-writer may use them for an owner-requested bounded task; every different authorized client treats them as unavailable and hands proposed changes to the human owner. The global Agents screen never promotes a client. A general memory names why it generalizes, and promotion copies the lesson into brain/ while retaining the memory with its promoted marker.",
  },
...
3. Read AGENTS.md and CLAUDE.md. Read .owdignore when present, then call resume_project with its exact Project UUID and context policy as the first OWD action. Until it returns, the writer role is unconfirmed; never infer it from session identity or Mind tool access. Treat "MDevolved resume project" and the legacy phrase "OWD resume project" as this receipt-based resume, with no reconnect or new approval. If no receipt exists, call open_project with the user's visible Project name.
