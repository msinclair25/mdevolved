...
OWD keeps a compact JSON vocabulary; RDF and a graph database are not required.
Edge direction is part of validation:

| OWD edge              | Subject class | Object class | W3C PROV meaning                                          |
| --------------------- | ------------- | ------------ | --------------------------------------------------------- |
...
- MCP Tasks remain experimental and are not a durable Project, Work Item,
  Attempt, or job record. A future negotiated adapter may wrap an OWD-owned job.
...
OWD does not expose an A2A service in V1. The documented mapping is lossy and
non-authoritative:

| OWD                 | A2A 1.0 analogue                         | Compatibility note                                                                     |
| ------------------- | ---------------------------------------- | -------------------------------------------------------------------------------------- |
| Project             | `context_id`                             | An A2A context cannot grant OWD Project access.                                        |
| Work Item           | `Task`                                   | OWD lifecycle and owner Decisions remain authoritative.                                |
| Work Packet         | input `Message` plus `Part` content      | Packet integrity, exact citations, and retained evidence remain OWD extensions.        |
| Attempt             | task execution/activity observation      | A2A Task state never replaces append-only OWD Attempts.                                |
| Artifact            | `Artifact`                               | Media/data can map; OWD hashes and recovery classification remain required.            |
...
| Decision/OwnerEvent | no core equivalent                       | Must remain OWD owner authority; an extension may reference it.                        |

A2A Message history is not OWD harness context and is never silently ingested.
