...
This page freezes the protocol boundary that MDevolved must satisfy before the
...
MDevolved exposes one remote, authenticated Streamable HTTP endpoint at `/mcp`.
...
completion support. Execution engines remain outside MDevolved.
...
durable results. Resume is not transcript or session replay. MDevolved keeps durable
...
advanced operations. None of these operations makes MDevolved an agent runtime or
...
MCP transport compatibility never widens MDevolved authority. Every tool call still
...
| Current named call/read/get with matching `Mcp-Name`           | JSON-RPC success or the ordinary MDevolved authorization result |
...
| JSON-RPC batch                                                 | Rejected; MDevolved does not execute any member                 |
| Request body over 64 KiB                                       | HTTP `413`; MDevolved does not execute it                       |
...
The body limit applies before JSON parsing and before MCP or MDevolved dispatch. The
transport must not inspect an unbounded clone of the request stream. Modern
header validation takes precedence over MDevolved's tool-specific challenge and
...
- Current and legacy clients share the same tools, resources, prompts, OAuth
  policy, revocation behavior, and MDevolved authorization checks.
...
> MDevolved implements authenticated remote MCP Streamable HTTP for the current
