...
proposed changes. The owner may explicitly transfer one bounded task only after
confirming the prior writer stopped. Agents do not infer a transfer from having
Obsidian or filesystem tools, and they do not need to be disconnected to remain
read-only.
