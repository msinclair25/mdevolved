...
6. Keep the sync client open while MDevolved automatically publishes the current searchable
   library.
7. Choose your existing agent in MDevolved, copy its one setup command, and run
   it once from the Project folder. Approve the exact Source when the browser
   opens.
...
## Advanced client recipes

The normal path is the single command generated by MDevolved. Use the manual
recipes below only when a harness cannot run that installer.
